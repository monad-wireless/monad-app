...
/**
 * SHA-256 over a byte array, in common Kotlin.
 *
 * Written here rather than pulled from a library because the build runs offline and the one
 * consumer — the IP-162 evidence manifest — needs a digest that three repositories compute
 * identically. FIPS 180-4, no options, no streaming: a session's artefacts are rendered as whole
 * byte arrays before upload, so a one-shot digest is the natural shape.
 *
 * Checked against the FIPS vectors in `Sha256Test` and against the shared fixture digests in the
 * IP-162 conformance test; if either disagrees, this file is wrong and the manifest is worthless.
 */
object Sha256 {
    ...
    /** Lower-case hex, 64 characters — the form every IP-162 document carries. */
